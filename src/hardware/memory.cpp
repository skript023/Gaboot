...
namespace gaboot
{
    ...
    double memory::memory_used()
    {
        this->get_memory();
#ifdef _WIN32
        auto current_memory = m_memory_info.ullTotalPageFile - m_memory_info.ullAvailPageFile;

        return this->bytes_to_gigabytes(current_memory);
#elif __linux__
        auto current_memory = m_total_memory - m_free_memory;

        return this->kilobytes_to_gigabytes(current_memory);
#endif
    }
