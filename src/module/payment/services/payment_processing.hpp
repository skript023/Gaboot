...
namespace gaboot
{
    ...
    #define TRANSACTION_FAILED(midtrans) return midtrans->status((HttpStatusCode)midtrans->m_data.status_code)->json();
    #define TRANSACTION_SUCCESS(midtrans) return midtrans->status((HttpStatusCode)midtrans->m_data.status_code)->json();
